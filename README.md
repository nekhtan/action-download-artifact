...
> If `commit` or `pr` or `branch` or `ref` or `run_id` or `workflow_conclusion` is not specified then the artifact from the most recent successfully completed workflow run will be downloaded.
...
- name: Download artifact
  ...
  with:
    ...
    branch: master
    # Optional, will use the specified ref (tag). Defaults to all refs
    ref: v1.0.0
